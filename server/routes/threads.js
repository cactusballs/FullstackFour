...
threadRouter.get("/:topic", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .send({ message: "No threads found for this topic" });
    }
...
threadRouter.get("/:topic/:tag", async (req, res) => {
  ...
  try {
    const sqlThreadPostsWithTags =
      "SELECT * FROM threads WHERE topic = ? and " + tagParam + " = 1";
    //SELECT * FROM threads where carers_tag =1 and topic = 'SEND';
    const [results] = await database.query(
      sqlThreadPostsWithTags,
      [topic],
      [tagParam]
    );
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .send({ message: "No threads found with this tag" });
    }
...
threadRouter.post("/create", async (req, res) => {
  ...
  if (!thread_title || !content || !topic || !user_name) {
    return res
      .status(400)
      .json({ message: "Title, content, topic, and user name are required" });
  }
...
threadRouter.post("/reply", async (req, res) => {
  ...
  if (!threadId || !content || !author) {
    return res
      .status(400)
      .json({ message: "Thread ID, content, and author are required" });
  }
  try {
    const sqlInsert = `
      ...
      VALUES (?, ?, ?, NOW())
    `;
    const values = [threadId, author, content];
